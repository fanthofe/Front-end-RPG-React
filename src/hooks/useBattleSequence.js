    ...
    useEffect(() => {
      ...
      if (mode) {
        ...
        switch (mode) {
          case 'attack': {
            ...
            (async () => {
              ...
              turn === 0
                ? setOpponentAnimation('static')
                : setPlayerAnimation('static');
                setAnnouncerMessage(`${receiver.name} takes ${damage} damages`);
              turn === 0
          ...
          case 'magic': {
            const magicalDamage = magic({ attacker, receiver });
            ...
            (async () => {
              ...
              turn === 0
                ? setOpponentAnimation('static')
                : setPlayerAnimation('static');
              setAnnouncerMessage(`${receiver.name} takes ${magicalDamage} magical damages`);

              turn === 0
                ? setOpponentHealth(h => (h - magicalDamage > 0 ? h - magicalDamage : 0))
                : setPlayerHealth(h => (h - magicalDamage > 0 ? h - magicalDamage : 0)); // We don't want a negative HP.
              await wait(2500);
          ...
          case 'heal': {
            ...
            (async () => {
              ...
              setAnnouncerMessage(`${attacker.name} recovered ${recovered} HP`);
              turn === 0
                ? setPlayerHealth(h =>
